CREATE OR REPLACE VIEW public.vista_pagos_totales
 AS
     ...
     LEFT JOIN ( SELECT f.orden_id,
           ...
           FROM facturas f
             RIGHT JOIN ( SELECT p.factura_id,
                  ...
                  WHERE (p.tipo::text = 'payment'::text OR p.proveedor_id = 4427 OR (p.proveedor_id IN ( SELECT proveedores.id
                           FROM proveedores
                          WHERE proveedores.oficio = true))) AND (p.state_id = 23 OR p.state_id = 27 OR p.state_id = 25)
                  GROUP BY p.factura_id) pp ON pp.factura_id = f.id
             LEFT JOIN ( SELECT p.factura_id,
                   ...
                   FROM pagos p
                     JOIN facturas f_1 ON f_1.id = p.factura_id
                     JOIN pagos_lineas pl ON pl.pago_id = p.id
                  WHERE p.tipo::text = 'impuestos'::text AND p.state_id <> 26 AND p.proveedor_id <> 4427 AND NOT (p.proveedor_id IN ( SELECT proveedores.id
                           FROM proveedores
          ...
          GROUP BY f.orden_id) pagado ON pagado.orden_id = o.id
     LEFT JOIN ( SELECT t.orden_id,
            round(sum(t.total), 2) AS total,
            round(sum(t.total / COALESCE(t.cot_dolar, 1::numeric)), 2) AS total_dolar
           FROM ( SELECT al.autorizado_id,
                    al.cot_dolar,
                    al.orden_id,
                        CASE
                   ...
                   FROM autorizado_lineas al
                     LEFT JOIN ( SELECT autorizado_linea_actas.autorizado_linea_id,
                            sum(COALESCE(autorizado_linea_actas.monto, 0::numeric)) AS total
                           FROM autorizado_linea_actas
                          GROUP BY autorizado_linea_actas.autorizado_linea_id) ala ON al.id = ala.autorizado_linea_id
                  GROUP BY al.id) t
...
ALTER TABLE public.vista_pagos_totales
    OWNER TO root;

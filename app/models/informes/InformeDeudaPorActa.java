...
public class InformeDeudaPorActa extends Model{
	...
	public static Model.Finder<Long,InformeDeudaPorActa> find = new Finder<Long,InformeDeudaPorActa>(Long.class, InformeDeudaPorActa.class);


	public static Pagination<InformeDeudaPorActa> page(String acta_numero,
    	...
    	Pagination<InformeDeudaPorActa> p = new Pagination<InformeDeudaPorActa>();
	...
	public static Map<String,Map<String,List<List<SqlRow>>>> getListaFinalDeudasDetallesReporte(Integer idProveedor,Boolean equipamiento,Integer idCuenta,boolean sinServicio){
		...
		if(idProveedor.compareTo(-1) == 0){
			...
			//listaProveedores.add(14733);//biomisiones
		...
		if(idCuenta != null) {
			...
			List<List<SqlRow>> listadoSqlRow = new ArrayList<List<SqlRow>>();
				...
				List<SqlRow> proveedorOperativa = InformeDeudaPorActa.getDeudaPorProveedorPorAntiguedadDetalles(listaProveedores,false,equipamiento,noDestacados,idCuenta,sinServicio);
			...
			listadoSqlRow = new ArrayList<List<SqlRow>>();
				...
				List<SqlRow> proveedorProfe = InformeDeudaPorActa.getDeudaPorProveedorPorAntiguedadDetalles(listaProveedores,true,equipamiento,noDestacados,idCuenta,sinServicio);
	...
	public static Map<String,List<List<SqlRow>>> getListaFinalDeudasResumenMensualReporte(Integer idProveedor,boolean equipamiento){
		...
		List<List<SqlRow>> listadoSqlRow = new ArrayList<List<SqlRow>>();
			...
			List<SqlRow> proveedorOperativa = InformeDeudaPorActa.getDeudaPorProveedorPorAntiguedadResumenMensual(false,false);
			if(proveedorOperativa.size() > 0){
				listadoSqlRow.add(proveedorOperativa);
			}
		...
		listadoSqlRow = new ArrayList<List<SqlRow>>();
			...
			List<SqlRow> proveedorProfe = InformeDeudaPorActa.getDeudaPorProveedorPorAntiguedadResumenMensual(true,false);
			if(proveedorProfe.size() > 0){
				listadoSqlRow.add(proveedorProfe);
			}
	...
	public static Map<String,List<SqlRow>> getListaFinalDeudasDetallesInstitucionReporte(){
		...
		//OPERTATIVAAAA
		List<SqlRow> operativa = InformeDeudaPorActa.getDeudaPorInstitucionPorAntiguedadDetalles(false);
		listaFinal.put("OPERATIVA", operativa);

		//PROFEEEEEE
		List<SqlRow> profe = InformeDeudaPorActa.getDeudaPorInstitucionPorAntiguedadDetalles(true);

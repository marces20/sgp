...
public class LiquidacionPuestosController extends Controller {
	...
	public static void pasarAprobado(Long idLiquidacionPuesto){
		...
		List<LiquidacionPuesto> lcx = LiquidacionPuesto.find.where()
				   ...
				   .ne("puestoLaboral.estado_id",Estado.PUESTO_LABORAL_CONTROLADO)
				   .endJunction()
				   .findList();
		...
		if(lcx.size() > 0) {
			...
			flash("error", "No se puede aprobar liquidaciones con fecha de baja en agentes o el puesto laboral no está aprobado.<br>"+f);

...
public class PlanificacionesController extends Controller {
	...
	public static void pasarAprobadoServicio(Long idRf){

		Planificacion rf = Ebean.find(Planificacion.class).select("id, estado_id,write_date,write_usuario_id").setId(idRf).findUnique();

		if(rf != null){
			rf.estado_id = new Long(Estado.PLANIFICIACION_APROBADO_SERVICIO);
			rf.write_date = new Date();
			rf.write_usuario_id = new Long(Usuario.getUsuarioSesion());
			rf.save();
	...
	public static void pasarAprobadoRrhh(Long idRf){

		Planificacion rf = Ebean.find(Planificacion.class).select("id, estado_id,write_date,write_usuario_id").setId(idRf).findUnique();

		if(rf != null){
			rf.estado_id = new Long(Estado.PLANIFICIACION_APROBADO_RRHH);
			rf.write_date = new Date();
			rf.write_usuario_id = new Long(Usuario.getUsuarioSesion());
			rf.save();
			flash("success", "Operación exitosa. Estado actual: Aprobado RRHH");
		} else {
			flash("error", "Parámetros incorrectos");
		}
	}

	public static void pasarAprobadoLiqui(Long idRf){

		Planificacion rf = Ebean.find(Planificacion.class).select("id, estado_id,write_date,write_usuario_id").setId(idRf).findUnique();

		if(rf != null){
			rf.estado_id = new Long(Estado.PLANIFICIACION_APROBADO_LIQUIDACIONES);
			rf.write_date = new Date();
			rf.write_usuario_id = new Long(Usuario.getUsuarioSesion());
			rf.save();
			flash("success", "Operación exitosa. Estado actual: Aprobado Liquidaciones");
		} else {
			flash("error", "Parámetros incorrectos");
		}
	}
	...
	public static Result imprimirPlanificacion(Long id) {
		 ...
		 try {
			 ...
			 String fileName = "planilla-"+rf.id;
			 ...
			 List<SqlRow> firstNovedades = novedades;
			 List<List<SqlRow>> subsNovedades = new ArrayList<>();
			 if(novedades.size() > 20 ) {
				 firstNovedades = new ArrayList<>(novedades.subList(0,19));

				 int qTotalSubLis = (novedades.size()-20)/20;
				 //int sizeTotalxx = (novedades.size()-20);
				 Logger.debug("novedades.size(): " +novedades.size());
				 Logger.debug("qTotalSubLis: " +qTotalSubLis);

				 do {
					 qTotalSubLis --;
					 List<SqlRow> listTmp = novedades.subList(20,40);
					 subsNovedades.add(listTmp);
				 } while (qTotalSubLis <= 0);
			 ...
			 String footer = "<footer clase=\"footer\" style=\"\">\n" +
			 		...
			 		"		        <td style=\"text-align: center;   border:none;\"><b style=\" padding-top:20px; border-top:1px solid #000000;\">Firma y Sello Jefe de RR.HH</b></td>\n" +
			 		"		        <td style=\"text-align: center;  border:none;\"><b style=\" padding-top:20px; border-top:1px solid #000000;\">Firma y Sello Resp. Jerárquicos</b></td>\n" +
			 		"			</tr>\n" +
			 		"		</tbody>\n" +
			 		"	</table>\n" +
			 ...
			 String footerPage = ReportePdf.getFooterPage(1, 1);

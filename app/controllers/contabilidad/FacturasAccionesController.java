...
public class FacturasAccionesController  extends Controller {
	...
	public static Result modificarNumeroFactura() {
		...
		Long id = new Long(request().body().asFormUrlEncoded().get("id")[0]);
		String fechaStr = request().body().asFormUrlEncoded().get("fecha")[0];
		...
		if(tipo_comprobante_str.isEmpty()) {
			flash("error", "Ingrese un Tipo comprobante");
			return ok(modalModificarNumeroFactura.render(d,id,f));
		}
		...
		if(!DateUtils.validateJavaDate(fechaStr)) {
			flash("error", "Fecha Incorrecta.");
			return ok(modalModificarNumeroFactura.render(d,id,f));
		}

		if(fechaStr.isEmpty()) {
			flash("error", "Ingrese una fecha");
			return ok(modalModificarNumeroFactura.render(d,id,f));
		}
		...
		try {
			...
			Date fecha = DateUtils.formatDate(fechaStr, "dd/MM/yyyy");

			FacturaDato fd = new FacturaDato();
			fd.tipo_comprobante_id = new Long(tipo_comprobante_str);

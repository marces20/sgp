...
public class FacturasReportesController extends Controller  {
	...
	public static Result OPCservicios() {
        ...
        try {
            ...
            context.put("math", new utils.NumberUtils());

           // $date.formatDate($l.factura.fecha_factura,"dd/MM/yyyy")

           // $math.roundTo
            //$math.roundTo(2, $f.dgr331)#set($totalDgr = $totalDgr + $f.dgr331)



            Factura ff = Factura.find.where().eq("id", 30725).findUnique();

            NumberUtils u = new utils.NumberUtils();
            Logger.debug("hhhhhhhhhhhhhhhhhhh222222222222333 fffffffff  "+ facturas.get(0).get("dgr331"));

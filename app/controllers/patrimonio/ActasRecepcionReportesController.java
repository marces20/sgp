...
public class ActasRecepcionReportesController extends Controller {
	...
	public static String getHeaderReporte(ActaRecepcion rf,BigDecimal totalRemitos,Integer cantRemitos) {
		...
		if( rf.ordenProvision.ordenCompra.cot_dolar != null &&  rf.ordenProvision.ordenCompra.cot_dolar.compareTo(BigDecimal.ZERO) > 0) {
	    	...
	    	textoConDolar =" - "+tm.titulo+" ";
	    	textoConDolar += utils.NumberUtils.moneda(total, rf.ordenProvision.ordenCompra.tipo_moneda);
	    	textoConDolar +=".BANCO NACION "+tm.signo+" 1 = "+utils.NumberUtils.moneda(cot);

	    	//textoConDolar +="<p style=\"font-size:12px;\">"+textoConDolar+"</p>";
		}

		String titulo = "Remitos";
		if(rf.ordenProvision.ordenCompra.tipo_orden.compareTo("servicio") == 0) {
			titulo = "Certificaciones";
		}

		/*return "<header><div class=\"header\"><table class=\"border_main cabecera_principal\" style=\"width: 100%;\">\n" +
		 		"             <tbody>\n" +
		 		"             	<tr>\n" +
		 		...
		 		"					  <p style=\"font-size:12px;\"><b>Proveedor: </b>"+rf.ordenProvision.ordenCompra.proveedor.nombre+" - "+
		 		"					  <b>Exp: </b>"+rf.ordenProvision.ordenCompra.expediente.getExpedienteEjercicio()+" <b>Total: </b>"+rf.ordenProvision.ordenCompra.expediente.getExpedienteEjercicio()+" </p> " +
		 		"					  <p style=\"font-size:12px;\"><b>INSTITUCION: </b>"+rf.ordenProvision.ordenCompra.deposito.nombre+"</p>\n" +
		 		"					  "+textoConDolar+""+
		 		"					  </div>"+
		 		"                 </td>\n" +
		 		"             	</tr>\n" +
		...
		return "<header><div class=\"header\"><table class=\"border_main cabecera_principal\" style=\"width: 100%;\">\n" +
 		...
 		"					</div>\n" +
 		"                 </td>\n" +
 		"                 <td style=\"width: 90%;border-left: 2px solid;border-left-width: 2px;padding-left:5px;text-align: center\">\n" +
 		"                     <h1>"+titulo+"</h1>" +
 		"					  <div style=\"text-align: center;\">" +
 		"					  <p style=\"font-size:12px;\"><b>Acta: </b>"+rf.getNombre()+" - "+
	...
	public static String getFooterReporte(BigDecimal total) {
		 String ret = "<footer clase=\"footer\" style=\"\">\n" +
	...
	public static Result imprimirCertificacionByActa(Long actaId) {

		try {
			 ActaRecepcion rf = ActaRecepcion.find.byId(actaId);

			 String fileName = "certificacion-"+rf.id;
			 int qpag = 0;
			 Map<String,String> datos = new HashMap<>();

			 String body = "";

			 List<CertificacionServicio> novedades = CertificacionServicio.find.where().eq("acta_id", actaId).orderBy("fecha_certificacion asc, id asc").findList();

			 BigDecimal totalRemitos = BigDecimal.ZERO;
			 Integer cantRemitos = novedades.size();
			 for(CertificacionServicio rtm :novedades) {
				 totalRemitos = totalRemitos.add(rtm.getTotal());
			 }

			 Logger.debug("xxxxxxxxxxxxxxxxxxx - "+cantRemitos);
			 Logger.debug("xxxxxxxxxxxxxxxxxxx - "+totalRemitos);
			// String periodo =rf.periodo.getMesAnioStringPeriodo();
			 String texto = "";
			 String header = getHeaderReporte(rf,totalRemitos,cantRemitos); /*"<header><div class=\"header\"><table class=\"border_main cabecera_principal\" style=\"width: 100%;\">\n" +
				 		"             <tbody>\n" +
				 		"             	<tr>\n" +
				 		"                 <td style=\"width: 10%;text-align: center\">\n" +
				 		"                     <div style=\"margin:15px;text-align: left\">\n" +
				 		...
				 		"					  <div style=\"text-align: center;\">" +
				 		"					  <p style=\"font-size:12px;\"><b>Acta: </b>"+rf.getNombre()+" - "+
				 		"					  <b>Exp: </b>"+rf.ordenProvision.ordenCompra.expediente.getExpedienteEjercicio()+" </p> " +
				 		"					  <p style=\"font-size:12px;\"><b>INSTITUCION: </b>"+rf.ordenProvision.ordenCompra.deposito.nombre+"</p>\n" +
				 		"					  <p style=\"font-size:12px;\"><b>Total Remitos: </b>"+utils.NumberUtils.moneda(totalRemitos)+" / <b>Cantidad:</b> "+cantRemitos+"</p>\n" +
				 		"					  </div>"+
				 		"                 </td>\n" +
				 		"             	</tr>\n" +
				 		"         	</tbody>\n" +
			...
			/*String footer = "<footer clase=\"footer\" style=\"\">\n" +
				 		...
				 		"		        <td style=\"text-align: center;  border:none;\"><b style=\" padding-top:20px; border-top:1px solid #000000;\">TOTAL:"+utils.NumberUtils.moneda(total)+"</b></td>\n" +
			 ...
			 List<List<CertificacionServicio>> subsNovedades = new ArrayList<>();
			 ...
			 if(novedades.size() > 75 ) {
				 ...
				 while(fin < novedades.size()) {
					 ...
					 List<CertificacionServicio> listTmp = novedades.subList(inicio,fin);
			 ...
			 for(List<CertificacionServicio> sbx : subsNovedades) {
				 ...
				 int z = 0;
				 for(CertificacionServicio sx :sbx) {
					 ...
					 novedadesTr  += "<td style='text-align:center;border:1px solid black; padding:5px; margin:0px;'><b>"+sx.periodo.nombre+"</b></td>"
					 			   + "<td style='text-align: center;border:1px solid black; margin:0px;'>"+ DateUtils.formatDate(sx.fecha_certificacion)+"</td>";


					 total = total.add(sx.getTotal());
				...
				// String header = header;
				 ...
				 //if(pag == qpag) {
				 //	footer = getFooterReporte(total) ;
				 //}else {
				 //	footer =  getFooterReporte(BigDecimal.ZERO);
				 //}


				 body += header+hoja+   getFooterReporte(total)   +footerPage;
			 }
			 //FINN PAAAGEEEEESSSS
			...
			/*String footer = "<footer clase=\"footer\" style=\"\">\n" +
			 ...
			 for(List<Remito> sbx : subsNovedades) {
				 ...
				 for(Remito sx :sbx) {
					 ...
					 novedadesTr  += "<td style='text-align:left;border:1px solid black; padding:5px; margin:0px;'><b>"+sx.numero+"</b></td>"
					 			   + "<td style='text-align: right;border:1px solid black; margin:0px;'>"+ DateUtils.formatDate(sx.fecha_remito)+"</td>";
